...
def _save_model(agent: A2CAgent, e_name: str, steps: int) -> str:
...
def train_a2c(
    ...
    logger = TrainingLogger(
        ...
        settings=settings,
        algo_name=AlgoName.A2C,
        algo_type=AlgoType.POLICY_GRADIENT,
    )

    previous_agent: A2CAgent | None = None
    ...
    while training_steps < settings.total_max_training_steps:
        ...
        while not done and training_steps < settings.total_max_training_steps:
            ...
            rollout_step_counter += 1

            should_optimize = (
                    rollout_step_counter >= settings.rollout_steps
                    or done
            )

            if should_optimize:
                agent.optimize_rollout(
                    next_obs=next_obs,
                    done=done,
                )
